...
export function createEmbed (submission: Submission): APIEmbed {
  logger.debug(`Creating embed for submission ${stringify.submission(submission)}`)
...
function createPublicEmbed (submission: CompletedSubmission): APIEmbed {
  ...
  return new EmbedBuilder()
    ...
    .setFields(
      ...
      { name: 'Source', value: createClickableURLString(submission.links.source), inline: true },
      { name: 'Other links', value: submission.links.other, inline: true },
      { name: 'Open in Replit', value: `[Click here](${getReplitURL(submission.links.source)})`, inline: true }
    )
...
function createProcessingEmbed (submission: ValidatedSubmission): APIEmbed {
  const votes =
    submission.votes
      .map((v) => `${v.voter.displayName} ${v.type} ${v.role}`)
      .join('\n') || 'None'
