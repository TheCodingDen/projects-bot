...
async function fetchReviewThread (threadId: Snowflake): Promise<ThreadChannel> {
  ...
  const reviewThread = await runCatching(async () =>
    await privateSubmissions.threads.fetch(threadId)
  )
...
async function fetchFeedbackThread (
  ...
  const feedbackThread = await runCatching(async () =>
    await publicShowcase.threads.fetch(threadId)
  )
...
async function fetchVotes (prismaVotes: PrismaVote[]): Promise<Vote[]> {
  return await Promise.all(
    prismaVotes.map(async (v) => {
      ...
      const voter = await runCatching(async () =>
        await config.guilds().current.members.fetch(v.voterId)
      )
...
async function fetchOriginalMessage (messageId: Snowflake): Promise<Message> {
  ...
  return await runCatching(async () => await privateSubmissions.messages.fetch(messageId))
}
...
export async function validatePendingSubmission (
  ...
  const validated: ValidatedSubmission = {
    ...
    // We assume it has no votes
    votes: []
  }

  logger.debug(`Resolve submission ${stringify.submission(validated)}`)
